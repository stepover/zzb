 ...
 * up over the course of a Route evaluation and finally converted to [[zzb.rest.RestResponse]]s by the
 ...
 */
case class MethodRejection(supported: RestMethod) extends Rejection
...
/**
 * Rejection created by an [[zzb.rest.authentication.HttpAuthenticator]].
...
case class AuthenticationFailedRejection(cause: AuthenticationFailedRejection.Cause,
                                         challengeHeaders: List[RestHeader]) extends Rejection
