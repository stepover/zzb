...
trait MarshallingContext { self ⇒
   ...
   */
  def marshalTo(entity: RestEntity, headers: RestHeader*)
...
class DelegatingMarshallingContext(underlying: MarshallingContext) extends MarshallingContext {
  ...
  def marshalTo(entity: RestEntity, headers: RestHeader*): Unit = underlying.marshalTo(entity, headers: _*)
  def handleError(error: Throwable): Unit = underlying.handleError(error)
  //def startChunkedMessage(entity: RestEntity, ack: Option[Any] = None, headers: Seq[RestHeader] = Nil)(implicit sender: ActorRef) =
...
trait ToResponseMarshallingContext { self ⇒
   ...
   */
  def marshalTo(response: RestResponse)
   ...
   * passed to the network.
   */
  //def startChunkedMessage(response: RestResponse, ack: Option[Any] = None)(implicit sender: ActorRef): ActorRef
